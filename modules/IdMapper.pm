...
sub refseq_get_symbol{
    my ($org, $list) = @_;#organism, $list=ref of the input gene list,$id_doc_type
    my @list = @$list;
    my $tax_ID;
    my %transform_hash;
    if($org eq 'human'){$tax_ID='9606'}else{$tax_ID='10090';}
    my @final_genes = ();
    my $transform_hash_file=$transform_hash_dir.'annotations/'.$tax_ID.'_refseq2rnaGeneNames.hash';
    my  %transform_hash=%{retrieve($transform_hash_file)};
    my %final_hash;
            foreach my $e (@list) {

            if (exists $transform_hash{$e}) {
                my @ids = @{$transform_hash{$e}};
                for my $hgncsymbl (@ids){
                 if (!($hgncsymbl eq "")and !($hgncsymbl =~ /^\s*$/)){
                $final_hash{$e}=$hgncsymbl;
                last;
                }#  if (($hgncsymbl eq "")or($hgncsymbl =~ /^\s*$/))
                $final_hash{$e}="-";
             } #  for my $hgncsymbl (@ids){
            } #  if (exists $transform_hash{$e}) {
               else{$final_hash{$e}="-";}#else
        } # foreach my $e (@list) {
