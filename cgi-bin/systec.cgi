...
my $mhd = 1;#boolean to decide if pvalue is needed
...
<!--#END_button list to sent out list-->
<ul class="accordion">
HEREDOCEND
         print $heredoc;
        ################################################
...
sub sub_table_view_new {
    ...
    foreach my $mirna_name (@keys) {
        ...
        foreach my $transcript_id (sort {$sorted_target_hash{$b}<=>$sorted_target_hash{$a}} keys %sorted_target_hash) {
            my $pvalue_final =1-$pvalue_hash{$mirna_name}{$transcript_id};
            $target_details .= "<tr>\n";
