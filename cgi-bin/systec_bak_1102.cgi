...
sub table_view {
    ...
    if($input_type eq "mirbase_name"||$input_type eq "mirbase_accession") {
        $result_hash = &DbmiRNA::GetMirTableViewInfo($input_type, $asc_or_desc, $session, $list_name,$query_db,$session_dir,\%toolselection,$priority_user);#field_name,asse or desc, $temp_database,$tmp_table,$query_database
        #%result_hash=%$result_hash;
        # %table_hash=%{$result_hash->{'mirna_hash'}};
    }
    else {
        $result_hash = &DbmiRNA::GetTargetTableViewInfo($input_type, $asc_or_desc, $session, $list_name,$query_db,$session_dir,\%toolselection,$priority_user);#pass the tool selection to file
